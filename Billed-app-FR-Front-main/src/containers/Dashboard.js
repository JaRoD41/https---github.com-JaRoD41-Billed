...
export default class {
	...
	handleShowTickets(e, bills, index) {
		...
		bills.forEach((bill) => {
			// Correction du bug 4 : les notes de frais ne s'affichent plus si l'on déplie une note de frais et que l'on clique sur une autre note de frais
			// j'ajoute un data-state à chaque note de frais et je contrôle sa valeur afin de déplier ou replier la note de frais
			if ($(`#open-bill${bill.id}`).data('expanded') !== true) {
				// Code pour déplier/replier la note de frais
				$(`#open-bill${bill.id}`).data('expanded', true)
